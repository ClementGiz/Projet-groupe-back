...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        self.stdout.write(
            self.style.WARNING("Nettoyage de la base de données...")
        )
        ...
        self.stdout.write(
            self.style.SUCCESS("Génération des données en cours...")
        )
        ...
        cours_data = [
            ...
            (
                "PROJ",
                "Projet Fullstack - Web / Java Spring Boot + Angular",
                105
            ),
        ]
        ...
        for index, cours in enumerate(cours_list[:5], start=1):
            CursusCours.objects.create(
                cursus=cursus_dev,
                cours=cours,
                ordre=index
            )
        ...
        for filiere in filieres:
            promotion_attente = Promotion.objects.create(
                filiere=filiere,
                nom=f"{filiere.code} - En attente",
                date_debut=fake.date_between(
                    start_date="-1y",
                    end_date="today"
                ),
                date_fin=fake.date_between(
                    start_date="+1y",
                    end_date="+2y"
                )
            )

            promotions_attente[filiere.id] = promotion_attente
        ...
        for filiere in filieres:
            ...
            for year in [2025, 2026]:
                ...
                start_date = fake.date_between(
                    start_date=f'-{2026-year+1}y',
                    end_date=f'-{2026-year}y'
                )
        ...
        for _ in range(4):
            ...
            formateur_profile = FormateurProfile.objects.create(
                user=user
            )
        ...
        for filiere in filieres:
            ...
            promotion_attente = promotions_attente[filiere.id]
        ...
        User.objects.create_user(
            username="refAdmin_test",
            email="refadmin@ecole.fr",
            first_name="Ref",
            last_name="Admin",
            password="refadminpassword",
            role=User.Role.REF,
            is_staff=False
        )
        cursus_cours_list = list(
            CursusCours.objects.all()
        )

        for promo in promotions:

            for cc in random.sample(
                cursus_cours_list,
                min(3, len(cursus_cours_list))
            ):
                ...
                CoursDonne.objects.create(
                    ...
                    promotion=promo,
                    cours=cc,
                    date_debut=promo.date_debut + timedelta(
                        days=random.randint(1, 30)
                    ),
                    date_fin=promo.date_debut + timedelta(
